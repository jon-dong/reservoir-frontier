...
def sierpinski_triangle(size=512, n_points=200000):
    ...
    v = np.array(
        [[0, 0], [size - 1, 0], [size // 2, int((size - 1) * np.sqrt(3) / 2)]],
        dtype=float,
    )
...
# %%
folder = "data/250130stability_frontier_data/"
hist_video = []
images = []
...
for idx in range(3):
    ...
    for threshold in thresh_list:
        ...
        print(
            f"dim utils : {dim_spore} \t dim  : {np.median(ret_spore_zero.slope)} \t dim edge : {np.median(ret_spore_edge.slope)}"
        )
        ...
        axs[1, 1].plot(
            log_scales_edges[min_idx:max_idx], log_count_edges[min_idx:max_idx]
        )
        axs[1, 1].plot(np.log(ret_spore_edge.size), np.log(ret_spore_edge.count))

        axs[1, 1].set_title(
            f"spore {np.median(ret_spore_zero.slope):.3f} edge {np.median(ret_spore_edge.slope):.3f}"
        )

...
def zooming_plot(folder):
    ...
    for idx, img in enumerate(hist_video):
        ...
        axs[idx_bin[idx][0], idx_bin[idx][1]].imshow(
            img.T, norm=matplotlib.colors.LogNorm(vmin=1e-4, vmax=1e-1)
        )
...
def fractal_dim_convergence_plots2(folder, final_dim_edge_list, final_spore_list):
    ...
    for dim_edge, ret_spore_zero, c in zip(
        final_dim_edge_list, final_spore_list, color
    ):
...
def final_plot_threshold_all(folder, threshold_list_number=32):
    ...
    for idx, file in enumerate(sorted(os.listdir(folder))):
        ...
        axs.append(
            fig.add_subplot(
                gs[:2, 2 * idx : 2 * (idx + 1)], sharey=None if idx == 0 else axs[0]
            )
        )
    ...
    axs.append(fig.add_subplot(gs[2:, 4:]))
    for dim_edge, ret_spore_zero, c, label in zip(
        final_dim_edge_list, final_spore_list, color, labels
    ):
        H_edges, log_count_edges, log_scales_edges = dim_edge
        ...
        axs[-1].axhline(H_edges, c=c)
        axs[-1].plot(
            ret_spore_zero.size, ret_spore_zero.slope, label=label, marker="d", c=c
        )
        axs[-1].set_xscale("log")
        axs[-1].legend()
        # axs[1].scatter(ret_spore_zero.size, ret_spore_zero.slope, marker='x', c=c)
    ...
    return final_spore_list, max_thresholds


if __name__ == "__main__":
    # fractal_dim_folder('250130stability_frontier_data/', title_plot='prova')
    folder = "data/fractal_fc/"
    final_spore_list, max_thresholds = final_plot_threshold_all(folder, threshold_list_number=32)
    fractal_lin_regs(final_spore_list, max_thresholds)
    #zooming_plot(folder)
